...
export default function NewTrainingPlanScreen() {
  ...
  const [training_plan_exercises, setTrainingPlanExercises] = useState<TrainingPlanExercise[]>([]) // Stores The Training Plans Exercises
  ...
  const handleDrop = (x:number, y:number, dragged_exercise:Exercise|TrainingPlanExercise|"custom_exercise"|"warm_up"):void => {
    ...
    if(dragged_exercise === "custom_exercise") {
      ...
      const new_exercise:TrainingPlanExercise = {
        ...
        order: training_plan_exercises.length,
        is_warm_up: false,
        is_custom_exercise: true
      }

      if(training_plan_slide !== "exercise") setTrainingPlanSlide("exercise") // Sets The Training Plan Slide
      setTrainingPlanExercises([...training_plan_exercises, new_exercise]) // Sets The Training Plans Exercises
      setActiveExerciseIndex(training_plan_exercises.length) // Sets The Active Exercise Index
    ...
    if(dragged_exercise === "warm_up") {
      ...
      if(training_plan_slide !== "exercise") setTrainingPlanSlide("exercise") // Sets The Training Plan Slide
      setTrainingPlanExercises([new_exercise, ...training_plan_exercises]) // Sets The Training Plans Exercises
    ...
    if("training_plan_key" in dragged_exercise) {
      if(!is_inside_drop_zone) {
        ...
        const updated_training_plan_exercises:TrainingPlanExercise[] = training_plan_exercises.filter((one_exercise:TrainingPlanExercise) => {
          return one_exercise.id !== dragged_exercise.id
        })

        setTrainingPlanExercises(updated_training_plan_exercises) // Sets The Training Plans Exercises
        setActiveExerciseIndex(0) // Sets The Active Exercise Index
        if(updated_training_plan_exercises.length === 0) setTrainingPlanSlide("drop_zone") // Sets The Training Plan Slide
      }
    ...
    // Exercise Selection Drag & Drop
      ...
      if(is_inside_drop_zone) {
        ...
        if(training_plan_exercises.some((one_exercise:TrainingPlanExercise) => one_exercise.id === dragged_exercise.id)) {
          const existing_exercise_index:number|null = training_plan_exercises.findIndex((one_exercise:TrainingPlanExercise) => one_exercise.id === dragged_exercise.id) || null // Gets The Existing Exercise
    ...
    <BackgroundContainer>
      <SafeAreaView style={[styles.safe_area, { flex: 1 }]}>
        ...
        >
          ...
          <NewTrainingPlan 
            onTrainingPlanExercisesUpdate={(training_plan_exercises:TrainingPlanExercise[]) => setTrainingPlanExercises(training_plan_exercises)}
            ...
            active_training_plan_day={active_training_plan_day}
            ...
            onTrainingPlanSlideUpdate={(training_plan_slide:TrainingPlanSlide) => setTrainingPlanSlide(training_plan_slide)}
            training_plan_slide={training_plan_slide}
          />
